...
const table = require('as-table').configure({ title: x => x.bright, delimiter: ' | '.dim.cyan, dash: '-'.bright.cyan })
...
function fpl (symbol, value) {
  let format = '+0,0[.]00000000'
  if (value < 0) {
    return [symbol, numbro(value).format(format)].join(' ').red
  } else if (value > 0) {
    return [symbol, numbro(value).format(format)].join(' ').green
  }
}
function fp (value) {
  let format = '+00.00%'
    ...
    try {
      ...
      if (this.started) {
        log(table([{
          ...
          // Summary
          'balance start': fk(this.currency, this.currencyBalanceConsolidatedStart).cyan,
          'balance current': fk(this.currency, this.currencyBalanceConsolidatedBase).magenta,
          'balance + p/l': fk(this.currency, this.currencyBalanceConsolidated).yellow,
          'p/l': fpl(this.currency, this.currencyBalanceConsolidatedBase - this.currencyBalanceConsolidatedStart),
          'p/l %': fp(this.currencyBalanceConsolidatedBaseDiff),
          'p/l % position': fp(plp),
          'p/l % consolidated': fp(this.currencyBalanceConsolidatedDiff),

          // Order stats
          'buy orders': `${this.ordersBuy.length}`.green,
          'sell orders': `${this.ordersSell.length}`.red
        }]))
      }
    } catch (e) {
      /* handle error */
