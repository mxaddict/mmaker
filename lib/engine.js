...
const ansicolo = require ('ansicolor').nice
...
function fp(value) {
  if (value < 0) {
    return numbro(value).format('0,0[.]00%').red
  }

  return numbro(value).format('+00.00%').green
}
...
module.exports = class Engine {
  ...
  async tickBalance () {
    ...
    if (!this.assetBalanceStart && !this.currencyBalanceStart) {
      this.assetBalanceStart = balance[this.asset]
      this.currencyBalanceStart = balance[this.currency]
    } else {
      this.assetBalance = balance[this.asset]
      this.currencyBalance = balance[this.currency]

      // Calculate the diff
      this.assetBalanceDiff = (this.assetBalance.total - this.assetBalanceStart.total) / this.assetBalanceStart.total
      this.currencyBalanceDiff = (this.currencyBalance.total - this.currencyBalanceStart.total) / this.currencyBalanceStart.total
    }
  ...
  async tickReport () {
    if (this.assetBalance && this.assetBalanceStart) {
      log.info(
        `${fk(this.asset, this.assetBalanceStart.total)}`.yellow, `/`, `${fk(this.asset, this.assetBalance.total)}`.magenta,
        `|`, `${fk(this.currency, this.currencyBalanceStart.total)}`.yellow, `/`, `${fk(this.currency, this.currencyBalance.total)}`.magenta,
        `|`, `${fp(this.assetBalanceDiff)} / ${fp(this.currencyBalanceDiff)}`,
      )
    }
