...
module.exports = class Engine {
    ...
    try {
      if (this.started && !this.running) {
        ...
        if (this.position) {
          ...
          if (this.position.long) {
            ...
            if (this.position.plp < this.positionPlTarget) {
              // Calculate the price
              price = this.position.base + (this.position.base * this.positionPlTarget)
            }
        ...
        if (this.orders.length === 0) {
          ...
          for (let i = newOrdersBuy.length, len = this.orderCountBuy; i < len; i++) {
          ...
          for (let i = newOrdersSell.length, len = this.orderCountSell; i < len; i++) {
            price += price * this.minWidthPercentIncrement
          ...
          if (this.position && this.position.long) {
            await this.exchange.privatePostOrderNewMulti({ orders: newOrdersSell.concat(newOrdersBuy) })
          } else {
            await this.exchange.privatePostOrderNewMulti({ orders: newOrdersBuy.concat(newOrdersSell) })
          }
      ...
      if (this.started) {
        ...
        log(table([{
          ...
          'orders size/buy/sell': [
            `${this.asset} ${fk(this.orders.length ? this.orders[0].amount : 0)}`.cyan,
  ...
  loadConf () {
    try {
      ...
      this.orderCountBuy = config.get('orderCountBuy') || 5
      this.orderCountBuyMin = config.get('orderCountBuyMin') || 2
      this.orderCountSell = config.get('orderCountSell') || 5
      this.orderCountSellMin = config.get('orderCountSellMin') || 2
      this.orderSize = config.get('orderSize') || 0
      this.orderSizeMultiplier = config.get('orderSizeMultiplier') || 0.05
