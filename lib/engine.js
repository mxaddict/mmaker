  ...
  async tickOrders () {
    try {
      if (this.started) {
        ...
        position.long = position.amount > 0

        // If pl acceptable, close the position
        if (parseFloat(position.pl) >= 0.003) {
          await this.exchange.privatePostOrderNew({
            symbol: this.market.replace('/', ''),
            amount: position.amount,
            price: (position.long ? this.bid: this.ask).toString(),
            exchange: 'bitfinex',
        ...
        if (
          // this.orderFair !== this.fair || // Are the orders still relevant to market?
          (this.orders.length && this.orders.length <= this.maxBullets + 1) // Do we have a balanced order set?
        ) {
