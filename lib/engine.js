...
module.exports = class Engine {
    ...
    try {
      if (this.started && !this.running) {
        ...
        if (this.position) {
          ...
          let amount = (this.position.long ? this.position.amount : this.position.amount * -1) / this.positionTargets.length
          ...
          if (this.position.long) {
            // Loop the targets
            for (var i = 0, len = this.positionTargets.length; i < len; i++) {
              // Calculate the price
              price = this.position.base + (this.position.base * this.positionTargets[i] / 100)
  ...
  loadConf () {
    try {
      ...
      this.positionTargets = config.get('positionTargets')
