    ...
    try {
      if (this.started) {
        ...
        if (
          this.orders.length &&
          (
            this.orders.length <= this.maxBullets + 1 ||     // Spread too big
            this.ordersBuy.length <= this.maxBullets / 2 ||  // Not enough BUYS
            this.ordersSell.length <= this.maxBulltes / 2 || // Not enough SELLS
            this.orders.length >= this.maxBullets * 2 ||     // Too many orders
            this.ordersBuy.length >= this.maxBullets ||      // Too many orders
            this.ordersSell.length >= this.maxBullets        // Too many orders
