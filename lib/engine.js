...
module.exports = class Engine {
  ...
  async start () {
    if (!this.conf) {
      log.n(log.ERROR(`Error loading "${this.argv.conf}"`))
      return false
    }

    // Check exchange
    if (!ccxt.exchanges.includes(this.exchange)) {
      log.n(log.ERROR(`Exchange "${this.exchange}" is not supported YET`))
      return false
    }

    // Load the exchange
    log.n(log.INFO(`Using Exchange "${this.exchange}"`))
    ...
    if (!this.markets.includes(this.market)) {
      log.n(log.ERROR(`Market "${this.market}" is not supported YET`))
      return false
    }

    // Save the marketInfo
    log.n(log.INFO(`Using Market "${this.market}"`))
  ...
  loadConf (confPath) {
    ...
    log.n(log.info(`Loading conf from "${confPath}"`))
