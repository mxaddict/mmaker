  ...
  async tickOrders () {
    try {
      if (this.started && !this.running) {
        ...
        let orderCountModBuy = (this.position ? (this.position.long ? 0 : this.positionTargets.length) : 0)
        let orderCountModSell = (this.position ? (this.position.long ? this.positionTargets.length : 0) : 0)

        // Check if we need to update orders
        if (
          this.orders.length &&
          (
            this.orders.length > (this.orderCountBuy + this.orderCountSell + this.positionTargets.length) ||
        ...
        if (this.position) {
          ...
          amount = (amount - this.orderSize * (this.position.long ? this.orderCountSell : this.orderCountBuy)) / this.positionTargets.length
