...
module.exports = class Engine {
  constructor (argv) {
    ...
    this.pollPrice = this.conf.pollPrice || 250            // How often do poll for price changes
    this.pollOrders = this.conf.pollOrders || 500          // How often do poll for order changes
    this.pollOrdersBuy = this.conf.pollOrdersBuy || 5000   // How often do poll for order changes
    this.pollOrdersSell = this.conf.pollOrdersSell || 5000 // How often do poll for order changes
    this.pollTrades = this.conf.pollTrades || 5000         // How often do poll for trade changes
    this.pollBalance = this.conf.pollBalance || 1000       // How often do poll for balance changes
    this.pollReport = this.conf.pollReport || 2500         // How often do poll for report changes

    // Get order settings
    this.minWidthPercent = (this.conf.minWidthPercent || 0.2) / 100
    this.minWidthPercentIncrement = (this.conf.minWidthPercentIncrement || 0.01) / 100
    this.maxBullets = this.conf.maxBullets || 4
    ...
    try {
      if (this.started) {
        ...
        if (this.orderBuyFair !== this.fair || this.ordersBuy.length !== this.maxBullets) {
          for (var i = 0, len = this.ordersBuy.length; i < len; i++) {
            await this.exchange.cancelOrder(this.ordersBuy[i].id, this.market)
          }
        }
        ...
        if (!this.ordersBuy.length) {
          ...
          for (let i = 0, len = this.maxBullets; i < len; i++) {
            ...
            await this.exchange.createLimitBuyOrder(this.market, this.orderSize, price)
          }
    ...
    try {
      if (this.started) {
        ...
        if (this.orderSellFair !== this.fair || this.ordersSell.length !== this.maxBullets) {
          for (var i = 0, len = this.ordersSell.length; i < len; i++) {
            await this.exchange.cancelOrder(this.ordersSell[i].id, this.market)
          }
        }
        ...
        if (!this.ordersSell.length) {
          ...
          for (let i = 0, len = this.maxBullets; i < len; i++) {
            ...
            await this.exchange.createLimitSellOrder(this.market, this.orderSize, price)
          }
  ...
  adjustOrderSize (price) {
    ...
    if (this.orderSize * price < this.marketInfo.limits.cost.min) {
      this.orderSize = this.marketInfo.limits.cost.min / price * 1.02
    }
  }
